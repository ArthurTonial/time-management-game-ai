...
def player_name(player_dir:str) -> str:
    """
    Removes leading 'agents.' or 'agents/' from the player directory
    :param player_dir:
    :return:
    """
    if player_dir.startswith('agents'):
        return player_dir[len('agents')+1:]  # +1 to account for the / or .

class Server(object):
    """
    Game server, implements a simple playing protocol based on function calls
    FIXME hangs on timeouts; should kill the process. (this? https://stackoverflow.com/questions/323972/is-there-any-way-to-kill-a-thread)
    or https://stackoverflow.com/questions/26063877/python-multiprocessing-module-join-processes-with-timeout
    FIXME does not register a loss for the player that times out
    """

    def __init__(self, game_type, p1_agent, p2_agent, delay, history, output, pace=0):
        """
        Initializes the Game server
        :param game_type: type of game to play (othello, tttm for tic-tac-toe misere, gomoku)
        ...
        if game_type not in {'othello', 'tttm', 'gomoku'}:
            raise ValueError(f"Unknown game type '{game_type}'. Allowed types are 'othello', 'tttm', and 'gomoku'")

        if game_type == 'othello':
            from src.game.othello.board import Board 
            from src.game.othello.gamestate import GameState
        elif game_type == 'tttm':
            from src.game.tttm.board import Board
            from src.game.tttm.gamestate import GameState
        elif game_type == 'gomoku':
            from src.game.gomoku.board import Board
            from src.game.gomoku.gamestate import GameState
    ...
    def write_output(self):
        ...
        # Use game name for XML root element
        game_name = self.state.game_name.lower().replace(' ', '-')
        root = ET.Element(f'{game_name}-match')
...
if __name__ == '__main__':
    ...
    parser.add_argument('game_type', type=str, choices=['tttm', 'othello', 'gomoku'],
                        help='Choose the game type: "tttm" (tic-tac-toe misere), "othello", or "gomoku"')
