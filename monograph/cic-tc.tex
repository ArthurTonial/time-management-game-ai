...
\begin{abstract}
% TODO: escrever resumo (máx. 500 palavras)
\end{abstract}
...
% TODO: escrever abstract (máx. 500 palavras)
\end{englishabstract}
...
O algoritmo Minimax e suas extensões (Seção~\ref{sec:minimax}) formalizaram essa estrutura e, combinados com funções de avaliação heurísticas cuidadosamente elaboradas, permitiram que programas de Xadrez atingissem nível grandmestre. Com o surgimento do Monte Carlo Tree Search (MCTS) \cite{coulom2006efficient,kocsis2006bandit}, porém, abriu-se uma nova fronteira: ao estimar o valor de posições por simulação estatística em vez de busca exaustiva, o MCTS dispensou a necessidade de funções de avaliação explícitas e demonstrou saltos de desempenho expressivos em jogos de alta complexidade, como o Go \cite{gelly2006modification}. Esse avanço culminou em marcos históricos como o AlphaGo e o AlphaZero, que combinaram MCTS com aprendizado profundo para superar os melhores jogadores humanos em Go e Xadrez.
...
\begin{itemize}
    ...
    \item Analisar os trade-offs e limitações de cada abordagem à luz dos resultados obtidos.
\end{itemize}
...
A busca em árvores de jogo é o paradigma central dos agentes automáticos para jogos de dois jogadores com informação perfeita. Esta seção apresenta o problema na sua formulação clássica — o algoritmo Minimax e sua principal otimização, a poda alfa-beta — que constituem a fundação histórica da área e o ponto de partida conceitual para compreender as motivações do MCTS. A limitação fundamental dessas abordagens em domínios de alta complexidade, discutida na Seção~\ref{sec:minimax_limits}, é precisamente o que motivou o desenvolvimento do MCTS como alternativa baseada em simulação.

Jogos de dois jogadores com informação perfeita e soma zero constituem um dos domínios mais estudados em Inteligência Artificial. Nesse modelo, dois agentes se alternam realizando movimentos em um estado compartilhado; o ganho de um implica a perda equivalente do outro. Xadrez, damas, Go e Gomoku são exemplos canônicos. A representação natural de uma partida é uma \emph{árvore de jogo}: cada nó corresponde a um estado e cada aresta a um movimento legal, com folhas representando estados terminais cujo valor é determinado pelo resultado da partida.

O algoritmo Minimax \cite{russell2016artificial} percorre essa árvore recursivamente, atribuindo a cada nó o melhor resultado possível sob jogo ótimo de ambos os lados. O jogador maximizador (\emph{MAX}) escolhe o movimento de maior valor; o minimizador (\emph{MIN}), o de menor:
...
O Monte Carlo Tree Search (MCTS) é uma família de algoritmos de busca que combina a expansão seletiva de uma árvore de jogo com a estimativa de valores por simulação estocástica \cite{browne2012survey}. Diferentemente do Minimax, o MCTS não requer função de avaliação: o valor de um estado é estimado pela proporção de vitórias obtidas em simulações (chamadas \emph{rollouts}) iniciadas a partir daquele estado.

O MCTS foi proposto de forma seminal por \citet{coulom2006efficient}, que introduziu o uso de simulações Monte Carlo combinadas com busca em árvore para jogos de tabuleiro. A variante UCT (\emph{Upper Confidence Bound for Trees}), que se tornou a formulação dominante, foi proposta por \citet{kocsis2006bandit} ao aplicar o critério UCB1 à seleção de nós na árvore. A aplicação ao Go por \citet{gelly2006modification} demonstrou saltos substanciais de desempenho em relação às abordagens anteriores, e o algoritmo passou a ser adotado em uma ampla variedade de domínios \cite{browne2012survey}.
...
Ao final de um orçamento de iterações ou tempo, o algoritmo retorna o movimento correspondente ao filho da raiz mais visitado, que tende a ser o de maior valor estimado.
...
\begin{figure}[htbp]
    ...
    \legend{Fonte: \citeauthor{browne2012survey} (\citeyear{browne2012survey})}
\end{figure}

\subsection{UCB1 e o dilema exploração-exploração}
\label{sec:ucb1}

O critério de seleção é o coração do MCTS. O problema pode ser formulado como um \emph{multi-armed bandit}: cada filho de um nó é um braço cujo valor real é desconhecido e deve ser estimado por tentativas sucessivas. O UCB1 (\emph{Upper Confidence Bound 1}), proposto por \citet{auer2002finite}, balanceia a exploração de nós promissores com a exploração de nós pouco visitados:
...
Essas modalidades impõem ao jogador — humano ou computador — o problema de \emph{alocação de tempo}: dado um orçamento total finito $T$, quanto tempo dedicar a cada movimento? Jogadores humanos experientes intuitivamente gastam mais tempo em posições críticas e menos em lances óbvios. A formalização dessa estratégia para agentes automáticos é o objeto de estudo desta seção.

\subsection{Tipos de estratégias de alocação de tempo}
\label{sec:timealloc_types}

A literatura identifica um espectro de estratégias de alocação de tempo para agentes de jogo. As principais categorias são descritas a seguir, em ordem crescente de sofisticação.

\textbf{Uniforme} (\emph{flat}): o tempo disponível é dividido igualmente entre os movimentos estimados restantes. Formalmente, se $T_r$ é o tempo restante e $\hat{N}$ é a estimativa de movimentos restantes para aquele jogador, o tempo alocado ao lance corrente é $t = T_r / \hat{N}$. Essa abordagem é simples, previsível e serve como baseline natural para comparações.
...
\textbf{Reativa} (ou \emph{baseada em criticidade}): detecta posições táticas especiais — ameaças imediatas de vitória ou derrota — e aloca tempo extra nessas situações. Requer um critério de detecção de criticidade, que pode ir desde padrões simples (como sequências de $k-1$ peças em linha) até avaliações mais elaboradas.
...
O ponto de partida é a propriedade \emph{anytime} do MCTS (Seção~\ref{sec:mcts_anytime}): dado um prazo $t_i$ segundos para o lance $i$, o agente executa o maior número possível de iterações dentro desse limite e retorna o movimento mais visitado. A qualidade da decisão é uma função crescente do tempo disponível — mais iterações implicam estimativas de valor mais precisas e maior profundidade efetiva de busca. Contudo, essa função não é linear: os ganhos de precisão seguem uma lei de retornos decrescentes. As primeiras iterações eliminam incerteza mais rapidamente, pois amostreiam regiões do espaço de busca ainda completamente desconhecidas. À medida que certas linhas de jogo acumulam visitas, iterações adicionais refinam estimativas já razoáveis, contribuindo cada vez menos para a qualidade da decisão final.

Essa característica tem uma consequência direta para o problema de alocação: \emph{redistribuir} tempo de posições de baixo retorno marginal para posições de alto retorno marginal pode aumentar a qualidade agregada das decisões sem alterar o orçamento total. Formalmente, se $Q(t)$ denota a qualidade esperada de uma decisão com $t$ segundos de busca, e se $Q$ é côncava (retornos decrescentes), então para dois lances $i$ e $j$ com importâncias distintas $v_i > v_j$, alocar mais tempo ao lance $i$ e menos ao $j$ pode aumentar $v_i \cdot Q(t_i) + v_j \cdot Q(t_j)$ em relação à alocação uniforme — desde que a diferença de importância seja suficientemente grande para compensar a concavidade de $Q$.

O argumento acima é a motivação central deste trabalho: se algumas posições do Gomoku têm $v_i \gg v_j$ — isto é, errar nelas é muito mais custoso do que errar em outras —, então uma política de alocação que identifique e priorize essas posições pode superar a alocação uniforme em taxa de vitórias. As estratégias propostas na Seção~\ref{sec:strategies} operacionalizam diferentes critérios para estimar essa importância relativa.

\subsection{Formulação do problema}
\label{sec:problem_formal}

Seja $M$ o número total de movimentos do jogador ao longo de uma partida e $T$ o orçamento total de tempo disponível. O problema de alocação de tempo por lance consiste em encontrar uma sequência $t_1, t_2, \ldots, t_M$ tal que:
...
e que maximize a taxa de vitórias esperada do agente em duelos de longa duração. Uma política de alocação $\pi$ mapeia o estado observável no turno $i$ — tempo restante, número do lance, estado do tabuleiro — para um tempo $t_i$:
...
Essa taxonomia tem uma implicação experimental relevante. Qualquer diferença de desempenho entre a política open-loop (flat) e as políticas closed-loop pode ser atribuída, ao menos em parte, ao uso da informação do tabuleiro como sinal de alocação. Entre as estratégias closed-loop, o grau de sofisticação da informação extraída cresce da proporcional (contagem agregada) para a por fase (classificação ordinal) e a crítica (detecção de padrões posicionais específicos).
...
A estratégia uniforme divide o orçamento restante pelo número estimado de movimentos restantes:
...
A estratégia proporcional mantém a mesma forma da Equação~\ref{eq:flat}, mas aplica um multiplicador que escala linearmente com o preenchimento do tabuleiro: lances iniciais, com tabuleiro esparso, recebem ligeiramente menos tempo que o flat ($0{,}8\times$), enquanto lances tardios, com o tabuleiro mais preenchido, recebem mais ($1{,}2\times$). O multiplicador cresce linearmente com o preenchimento, mantendo o orçamento global próximo ao flat em média. A hipótese é que lances tardios — com fator de ramificação menor e posições mais definidas — se beneficiam de mais tempo, ao passo que lances de abertura, com espaço de busca enorme e pouco retorno marginal por iteração adicional, podem ser resolvidos com menos tempo.

\subsection{Por Fase (\emph{Phase})}
\label{sec:strategy_phase}

A estratégia por fase classifica o estado da partida em fases — abertura, meio do jogo e final — e aplica um multiplicador distinto sobre a alocação uniforme em cada fase:
...
O multiplicador $\mu_{\text{meio}} > 1$ destina ao meio do jogo uma parcela proporcionalmente maior do orçamento, enquanto $\mu_{\text{abertura}} < 1$ e $\mu_{\text{final}} < 1$ compensam essa redistribuição. A justificativa apoia-se diretamente na estrutura do Gomoku: é no meio do jogo que emergem as ameaças táticas de 3-em-linha e 4-em-linha que decidem a maioria das partidas. Investir mais tempo nessa fase aumenta a probabilidade de o MCTS identificar e responder corretamente a essas ameaças, enquanto a abertura e o final — por razões distintas — apresentam menor retorno esperado por unidade de tempo adicional.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lllp{5cm}}
        ...
        \hline
        Flat        & Open-loop   & Tempo restante           & Baseline; sem cenário de ganho específico \\
...
Do ponto de vista de risco orçamentário, flat é a mais conservadora: aloca exatamente $T_r / \hat{N}_r$ por lance. A proporcional varia entre $0{,}8$ e $1{,}2$ vezes esse valor, mantendo o orçamento global próximo ao flat em média. As estratégias por fase e crítica introduzem multiplicadores mais amplos (até $1{,}5\times$ e $2{,}0\times$, respectivamente) que podem consumir uma fração desproporcional do orçamento se ativados frequentemente — o que motiva a inclusão de fatores de desconto ($\mu < 1$) nos lances não críticos para manter o balanço orçamentário ao longo da partida.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/time_profiles}
    \caption{Perfil conceitual de tempo alocado por lance ao longo de uma partida para cada uma das quatro estratégias. A estratégia flat produz uma linha aproximadamente horizontal; a proporcional cresce suavemente; a por fase exibe um platô elevado no meio do jogo; a crítica apresenta picos nos lances em que ameaças táticas são detectadas.}
...
\begin{itemize}
    \item \texttt{place\_marker(player, row, col)}: registra uma peça e atualiza o atributo \texttt{last\_move = (row, col, player)}, usado pelo rollout do MCTS.
...
O rollout simula uma partida a partir do nó expandido até um estado terminal ou até o limite de 200 movimentos. Em vez de escolher movimentos uniformemente ao acaso, o rollout adota um viés de localidade: prefere movimentos dentro de um raio $r = 5$ ao redor da última peça colocada no tabuleiro. Formalmente, dada a última peça em $(r_{\text{last}}, c_{\text{last}})$, o conjunto preferencial de movimentos é:
...
Se $A_{\text{local}} \neq \emptyset$, o rollout escolhe uniformemente em $A_{\text{local}}$; caso contrário, escolhe em $A$ inteiro. Esse viés reduz efetivamente o espaço de busca durante a simulação — movimentos distantes de qualquer peça são raramente relevantes no Gomoku — e acelera a convergência das estimativas de valor sem requerer uma função de avaliação heurística.

\subsection{Retropropagação}
\label{sec:impl_backprop}

A retropropagação percorre o caminho da folha até a raiz de forma iterativa. O resultado do rollout ($1{,}0$ para vitória, $0{,}0$ para derrota, $0{,}5$ para empate) é invertido a cada nível — $\text{resultado} \leftarrow 1{,}0 - \text{resultado}$ — para refletir a alternância de perspectivas entre jogadores consecutivos na árvore.
...
A estratégia flat usa um estimador constante do número de movimentos restantes:
...
A estratégia proporcional utiliza o mesmo estimador $\hat{N}_r$ da estratégia flat, mas aplica um multiplicador que escala linearmente com o preenchimento do tabuleiro:
...
O fator de desconto $0{,}9$ nos lances não críticos compensa o orçamento extra consumido nas posições críticas, mantendo o balanço total próximo ao da estratégia flat ao longo de uma partida típica.
...
A cada lance, o runner registra quatro métricas: tempo alocado pela política ($t_i^{\text{eff}}$), tempo efetivamente consumido pelo MCTS, tempo restante após o lance e o movimento escolhido. Ao final de todas as partidas, os resultados são serializados em JSON com três seções:
...
Cada estratégia foi avaliada em um torneio de $N = 400$ partidas contra o baseline flat, com orçamento de 60 segundos por jogador por partida. Para controlar o viés do primeiro movimento — Black possui vantagem estrutural em Gomoku, conforme discutido na Seção~\ref{sec:gomoku_rules} — as 400 partidas foram divididas igualmente entre as duas cores: 200 partidas com a estratégia em avaliação jogando como Black e 200 como White.
...
A hipótese nula $H_0\colon p = 0{,}5$ — a estratégia tem a mesma taxa de vitórias que o baseline — foi testada pelo escore $z$:
...
Não foram observados empates em nenhuma das 1200 partidas realizadas, o que é consistente com a propriedade do Gomoku de que o jogo ótimo resulta sempre em vitória do primeiro jogador \cite{allis1994searching}.

% ---
\section{Visão Geral dos Resultados}
\label{sec:results_overview}

A Tabela~\ref{tab:results_main} apresenta as taxas de vitórias de cada estratégia, desagregadas por cor e no total, com os respectivos intervalos de confiança e estatísticas de teste.

\begin{table}[htbp]
    \centering
    \caption{Taxas de vitórias das estratégias contra o baseline flat ($N = 400$ partidas por estratégia, 200 por cor). IC: intervalo de confiança de 95\%. Significância: *** $p < 0{,}001$, ** $p < 0{,}01$, n.s.\ não significativo.}
    \label{tab:results_main}
    \begin{tabular}{llrrrlr}
        \hline
        \textbf{Estratégia} & \textbf{Cor} & \textbf{Vitórias} & \textbf{Partidas} & \textbf{Taxa} & \textbf{IC 95\%} & \textbf{Sig.} \\
        \hline
        Crítica      & Black & 123 & 200 & 61,5\% & [54,8\%; 68,2\%] & **  \\
                     & White & 124 & 200 & 62,0\% & [55,3\%; 68,7\%] & *** \\
                     & Total & 247 & 400 & \textbf{61,8\%} & [57,0\%; 66,5\%] & *** \\
        \hline
        Por Fase     & Black &  89 & 200 & 44,5\% & [37,6\%; 51,4\%] & n.s.\\
                     & White &  72 & 200 & 36,0\% & [29,3\%; 42,7\%] & *** \\
                     & Total & 161 & 400 & \textbf{40,2\%} & [35,4\%; 45,1\%] & *** \\
        \hline
        Proporcional & Black & 103 & 200 & 51,5\% & [44,6\%; 58,4\%] & n.s.\\
                     & White & 106 & 200 & 53,0\% & [46,1\%; 59,9\%] & n.s.\\
                     & Total & 209 & 400 & \textbf{52,2\%} & [47,4\%; 57,1\%] & n.s.\\
        \hline
    \end{tabular}
\end{table}

Os resultados dividem-se em três categorias distintas: a estratégia crítica supera significativamente o baseline; a estratégia por fase é significativamente inferior; e a estratégia proporcional é indistinguível do baseline. A Figura~\ref{fig:results_winrates} ilustra essa comparação visualmente.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{images/results_winrates}
    \caption{Taxas de vitórias por estratégia e cor, com intervalos de confiança de 95\%. A linha tracejada em 50\% indica o nível do baseline flat. A estratégia crítica supera consistentemente o baseline em ambas as cores; a por fase fica consistentemente abaixo; a proporcional mantém-se próxima de 50\%.}
...
A estratégia crítica obteve 61,8\% de vitórias no total ($z = +4{,}70$, $p < 0{,}001$), resultado altamente significativo. O achado mais notável é a \textbf{simetria entre cores}: 61,5\% como Black e 62,0\% como White, uma diferença de apenas 0,5 pontos percentuais. Em Gomoku, White possui uma desvantagem estrutural que tipicamente resulta em taxas de vitória próximas a 40\% contra um oponente de força equivalente. O fato de a estratégia crítica obter 62\% como White — jogando contra a estratégia flat como Black — indica que a detecção de ameaças é capaz de reverter completamente essa desvantagem.

A análise da distribuição de tempo revela que a estratégia crítica gastou em média 0,878 s/lance, contra 0,796 s/lance do flat — uma diferença de aproximadamente 10\%. Esse acréscimo não é uniforme: ameaças táticas foram detectadas em \textbf{16,4\% dos lances} (aproximadamente 1 em cada 6), nos quais o tempo alocado dobrou para cerca de 1,6 s, enquanto os demais lances receberam $0{,}9\times$ da alocação flat. O sinal de detecção de ameaças é, portanto, esparso mas de alta precisão: identifica exatamente os lances em que um erro é imediatamente fatal e concentra recursos neles.

Esse resultado valida a hipótese central da estratégia crítica (Seção~\ref{sec:strategy_critical}): erros em posições com ameaça de 4-em-linha aberta têm custo irreparável, e investir tempo adicional nesses lances — ainda que em apenas 16\% dos casos — eleva significativamente a taxa de vitórias.

\subsection{Estratégia por Fase}
\label{sec:results_phase}

A estratégia por fase obteve 40,2\% de vitórias no total ($z = -3{,}90$, $p < 0{,}001$), sendo significativamente \emph{inferior} ao baseline. O resultado é paradoxal à primeira vista: a estratégia por fase gastou em média 0,900 s/lance — 13\% a mais que o flat — e ainda assim obteve menor taxa de vitórias. O problema está na distribuição temporal, não no volume total.

A Tabela~\ref{tab:phase_time} mostra a alocação média por faixa de lances do jogador para a estratégia por fase e para o flat.

\begin{table}[htbp]
    \centering
    \caption{Tempo médio alocado por faixa de lance do jogador: estratégia por fase vs.\ flat (partidas como Black, $n = 200$).}
...
O orçamento é gasto em excesso na abertura e no meio do jogo — fase em que o multiplicador de 1,5$\times$ eleva o gasto por lance em 36\% acima do flat — esgotando as reservas disponíveis para os lances finais. Quando o tabuleiro atinge o limiar de final ($p \geq 0{,}45$), dois efeitos negativos se combinam: (i) o orçamento residual já está reduzido pelo excesso de gastos anteriores, e (ii) o multiplicador de final ($\mu = 0{,}55$) desconta adicionalmente esse orçamento já reduzido. O resultado é que os lances 41+, onde as partidas costumam ser decididas, recebem 27\% menos tempo que o flat.

A assimetria entre as cores reforça essa interpretação. White joga os lances de resposta em posições com tabuleiro mais preenchido, justamente quando o desconto de final está ativo. Isso explica a queda da taxa de vitórias de 44,5\% como Black para 36,0\% como White.
...
A estratégia proporcional obteve 52,2\% de vitórias no total ($z = +0{,}90$, $p = 0{,}37$), resultado estatisticamente indistinguível do baseline com 400 partidas. A análise temporal revela que a estratégia proporcional gastou sistematicamente \textbf{menos} tempo que o flat: 0,758 s/lance contra 0,796 s/lance, uma redução de aproximadamente 5\%.
...
A redistribuição de tempo é, portanto, mais tênue na prática do que na intenção de projeto, e seus efeitos não são detectáveis com 400 partidas. O resultado é neutro: a estratégia proporcional não prejudica nem beneficia o desempenho em relação ao flat.
...
Esse gradiente sugere que, neste domínio e com este orçamento de tempo, \textbf{a qualidade do sinal de alocação importa mais do que o volume total de tempo redistribuído}. A estratégia crítica usa um sinal binário aplicado a apenas 16\% dos lances — mecanismo simples, mas altamente informativo. A estratégia por fase distribui o orçamento de forma mais contínua mas com menor acurácia na identificação dos lances decisivos.

\subsection{O efeito da cor na estratégia crítica}
\label{sec:discussion_color}

A simetria observada na estratégia crítica — 61,5\% como Black e 62,0\% como White — merece atenção específica. Ao anular a desvantagem estrutural de White, a estratégia crítica demonstra que a vantagem do primeiro movimento em Gomoku pode ser contrabalançada por uma política de alocação de tempo mais precisa. White, que tipicamente precisa reagir à iniciativa de Black, se beneficia proporcionalmente da detecção de ameaças: bloquear uma sequência de 4-em-linha no tempo correto é, para White, frequentemente a única forma de não perder no lance seguinte.

\subsection{Limitações}
\label{sec:discussion_limitations}

Os experimentos apresentam algumas limitações que devem ser consideradas na interpretação dos resultados. Primeiro, o único baseline utilizado foi a estratégia flat; comparações diretas entre as estratégias propostas não foram realizadas, o que impede afirmações sobre ordenação relativa entre elas. Segundo, todos os experimentos utilizaram um único orçamento de 60 segundos por jogador; resultados podem diferir com orçamentos distintos, particularmente para a estratégia crítica, cujo ganho depende da frequência de ameaças detectadas. Terceiro, ambos os agentes utilizam o mesmo motor MCTS; contra agentes com capacidades de busca diferentes, os efeitos das estratégias de tempo podem variar. Por fim, os resultados são específicos ao Gomoku; domínios com estrutura tática distinta podem apresentar padrões diferentes de resposta às estratégias de alocação. Uma direção natural de trabalho futuro é estender o sistema a outros jogos de tabuleiro — como Go ou Hex — e a outros agentes de busca, explorando se o ganho da estratégia crítica se generaliza para domínios com padrões táticos diferentes, e se a arquitetura extensível proposta facilita essa comparação.
...
O Monte Carlo Tree Search é, por natureza, um algoritmo \emph{anytime}: dado mais tempo, produz decisões de maior qualidade. Em contextos competitivos com orçamento fixo por partida, porém, esse recurso é finito e deve ser distribuído entre todos os lances. A questão investigada neste trabalho é se — e como — a qualidade dessa distribuição influencia a taxa de vitórias: alocar tempo de forma desigual, concentrando-o nos momentos mais importantes da partida, traz ganho mensurável em relação à alocação uniforme?

Para responder a essa questão, foram propostas e implementadas quatro estratégias de alocação de tempo sobre um agente MCTS aplicado ao Gomoku: a uniforme (\emph{flat}), que serve como baseline; a proporcional, que escala a alocação com o preenchimento do tabuleiro; a por fase, que aplica multiplicadores distintos conforme a fase do jogo; e a crítica, que detecta ameaças táticas imediatas e dobra o tempo alocado nesses lances. As estratégias foram comparadas em um torneio controlado de 400 partidas por estratégia contra o baseline flat, com orçamento de 60 segundos por jogador e alternância de cores para controlar o viés do primeiro movimento.

Os resultados dividem-se em três padrões distintos. A estratégia crítica obteve 61,8\% de vitórias ($z = +4{,}70$, $p < 0{,}001$), resultado consistente entre as duas cores (61,5\% como Black, 62,0\% como White). Notavelmente, o mecanismo de detecção de ameaças foi ativado em apenas 16,4\% dos lances — o suficiente para reverter completamente a desvantagem estrutural do jogador branco. A estratégia por fase obteve 40,2\% de vitórias ($z = -3{,}90$, $p < 0{,}001$), sendo significativamente inferior ao baseline: o multiplicador elevado no meio do jogo esgota o orçamento antes dos lances decisivos do final, penalizando especialmente o jogador branco (36,0\%). A estratégia proporcional, com 52,2\% ($p = 0{,}37$), é estatisticamente indistinguível do flat — a redistribuição real é mais tênue do que o projeto sugere, pois o multiplicador permanece abaixo de 1,0 na maior parte da partida.

O achado central que atravessa os três resultados é que \textbf{a qualidade do sinal de alocação importa mais do que o volume de tempo redistribuído}. A estratégia crítica redistribui tempo em apenas 16\% dos lances com um sinal binário simples, mas de alta precisão; a por fase redistribui ao longo de toda a partida com um sinal mais elaborado, porém menos acurado. A hipótese do trabalho — de que alocação inteligente supera o baseline flat — é confirmada parcialmente: vale quando o sinal é específico o suficiente (crítica), mas não quando é agregado (proporcional) ou mal calibrado (por fase).
