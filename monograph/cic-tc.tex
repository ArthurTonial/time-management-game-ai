...
\textbf{No código:} Durante a etapa de implementação da infraestrutura do projeto — que engloba o motor do jogo Gomoku, o agente MCTS e o \emph{runner} de torneios —, assistentes de IA foram pontualmente empregados. O uso se deu no auxílio à depuração de erros (\emph{debugging}), na refatoração de blocos lógicos específicos e em consultas rápidas sobre sintaxe de bibliotecas. A ferramenta atuou como um recurso de consulta interativa, acelerando a resolução de gargalos técnicos isolados de programação.
