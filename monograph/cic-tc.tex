...
\subsection{O algoritmo Minimax}
\label{sec:minimax_alg}

O algoritmo Minimax \cite{russell2016artificial} percorre essa árvore em profundidade, atribuindo a cada nó um valor que reflete o melhor resultado possível sob jogo ótimo de ambos os lados. O jogador maximizador (\emph{MAX}) escolhe o movimento de maior valor; o minimizador (\emph{MIN}) escolhe o de menor. O valor de um nó é definido recursivamente:
...
Para jogos com espaço de busca finito, o Minimax é teoricamente ótimo: garante o melhor resultado possível contra um oponente perfeito. Na prática, porém, a busca exaustiva é inviável para jogos com fator de ramificação elevado. O Xadrez possui fator de ramificação médio de aproximadamente 35 e profundidade média de partida de 80 lances, resultando em uma árvore com $35^{80}$ folhas — muito além da capacidade computacional disponível \cite{shannon1950programming}. Para contornar esse problema, a busca é limitada a uma profundidade predefinida, e os nós não terminais são avaliados por uma função heurística $h(n)$ que estima a utilidade do estado.

\subsection{Poda Alfa-Beta}
\label{sec:alpha_beta}

A poda alfa-beta \cite{knuth1975analysis} é a otimização mais relevante do Minimax. Ela mantém dois valores ao longo da busca: $\alpha$, o melhor resultado garantido para MAX nos ancestrais do nó atual, e $\beta$, o melhor resultado garantido para MIN. Sempre que $\alpha \geq \beta$ em um nó, os ramos restantes desse nó são descartados (\emph{podados}), pois não podem influenciar a decisão do ancestral.

No melhor caso, com os movimentos ordenados de forma ótima, a poda alfa-beta reduz a complexidade de $O(b^d)$ para $O(b^{d/2})$, efetivamente dobrando a profundidade de busca alcançável \cite{russell2016artificial}. Na prática, a ordem dos movimentos não é ótima, e o ganho situa-se entre os dois extremos. Ainda assim, a poda alfa-beta permitiu que programas de Xadrez atingissem níveis sobre-humanos de jogo combinada com funções de avaliação cuidadosamente elaboradas.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/alfabeta_pruning.png}
    \caption{Ilustração da poda alfa-beta sobre uma árvore Minimax. Os ramos marcados em cinza, com um $X$ vermelho na frente, são podados por não poderem influenciar a decisão da raiz, dado os limites $\alpha$ e $\beta$ já estabelecidos.}
